...
const MessageButton = ({
  ...
  useEffect(() => {
    ...
    if (time - lastMessageTime <= 3600000) {
      setTimeDisplay('minutesAgo');
      setTime(time - lastMessageTime);
    } else if (time - lastMessageTime <= 86400000) {
      setTimeDisplay('today');
      setTime(time - lastMessageTime);
    } else if (time - lastMessageTime <= 604800000) {
      setTimeDisplay('thisWeek');
      setTime(time - lastMessageTime);
    } else if (time - lastMessageTime <= 31536000000) {
  ...
  return (
    <button className="message-button-wrapper " onClick={setterFunc}>
      {notifications && (
        ...
        {time && timeDisplay && (
          <div className="msg-btn-info-bottom">
            {timeDisplay === 'minutesAgo' && (
              <p className="grn-btn-text">{Math.floor(time / 60000)} minutes ago</p>
            )}
            {timeDisplay === 'today' && (
              <p className="grn-btn-text">{Math.floor(time / 3600000)} hours ago</p>
            )}
            {timeDisplay === 'thisWeek' && (
              <p className="grn-btn-text">{Math.floor(time / 86400000)} days ago</p>
            )}
            {timeDisplay === 'thisYear' && (
              <p className="grn-btn-text">
                {lastMessageMonth.slice(0, 3)} {lastMessageDay}
              </p>
            )}
            {timeDisplay === 'longTime' && (
              <p className="grn-btn-text">
                {lastMessageMonth.slice(0, 3)} {lastMessageDay} {lastMessageYear}
              </p>
            )}
          </div>
        )}
