...
const Dropdown = ({ list, selection, title, setSelection }) => {
  ...
  return (
    <div
      className={expand ? 'dropdown-wrapper drop-active-back' : 'dropdown-wrapper'}
    ...
    >
      ...
      <div className={expand ? 'drop-menu-container' : 'drop-menu-container drop-shrink'}>
        {list.map((item, index) => (
          <div
            key={index}
            className={item === selection ? 'selectionButton drop-active' : 'selectionButton'}
            onClick={() => setSelection(item)}
          >
            <div
              className={item === selection ? 'selected-border' : 'selected-border transborder'}
            ></div>
